...
We position Dimensional Escape as a specific manifestation of Goodhart's Law~\cite{gao2023scaling, skalse2022defining} in low-rank-scored, high-dimensional latent spaces, generalizing the obfuscated-activations regime Bailey et al.~\cite{bailey2024obfuscated} characterize for gradient-trained input attacks. The novelty we claim is not a new phenomenon class but a specific empirical setting (inference-time KV-cache MCTS with surrogate-reward scoring) and a two-mechanism characterization of how the search satisfies the proxy without satisfying the objective (prior-amplification at small model scale, greedy-collapse at larger scale; §\ref{sec:exp1}--\ref{sec:exp2}).
...
Experiments were conducted on open-weight models in isolated environments; no production systems or real users were targeted. The Croissant~1.1 metadata package (§\ref{sec:future-work}) documents per-item runtime evidence artifacts from Experiments~1--2 with provenance and usage-constraint fields.
...
\bibitem{damon2025}
X.~Zhang, X.~Yin, D.~Jing, et al.
\newblock DAMON: A Dialogue-Aware MCTS Framework for Jailbreaking Large Language Models.
...
\newblock In \emph{Proceedings of the 57th Annual Meeting of the Association for Computational Linguistics (ACL)}, 2019. \emph{arXiv:1905.07830}.
