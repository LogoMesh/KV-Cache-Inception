...
For comparison, standard parallel MCTS requires $O(b^d \cdot M_{\text{KV}})$ memory. At the 20B scale---where the open-weight \texttt{gpt-oss-20b} architectural specifications imply $M_{\text{KV}} \approx 162$~GB per cache snapshot (Track~D side-finding)\footnote{Phase~2 implementation constant factor $\approx 2$--$3 \cdot M_{\text{KV}}$ is reported in Appendix~\ref{sec:appendix-impl-notes} item~(iv); Proposition~\ref{prop:memory}'s branching-factor independence holds across these constants.}---standard parallel MCTS with $b = 3$, $d = 5$ would require approximately $39$~TB ($3^5 \cdot 162$~GB), % [REVISION | §6.2-projection-footnote-delete-Day10-batch3 | 2026-05-15 | Day-10 Batch 3 page-budget cut #19 (Pass K speculation-vs-page-cost): deleted the Day-8-evening footnote elaborating Track D's measurement scope + the 20B projection. The body sentence "is therefore a projection from smaller-scale measurements rather than a direct 20B observation" retains the asymmetric-hedging fix the footnote previously documented; further Track D protocol detail is preserved at §A.4 Track D, accessible to a reviewer who wants the measurement scope. Per Day-9 page-budget cut plan §3 rank #19 (LOW risk, save 0.10 pp). — pending audit]
while reversible MCTS is projected to require approximately $(1 + 3.04) \cdot M_{\text{KV}} \approx 654$~GB total---the baseline cache ($\approx 162$~GB) plus accumulator overhead of $3.04 \cdot M_{\text{KV}} \approx 493$~GB. The implied ${\sim}60\times$ reduction in working-set memory at $(b, d) = (3, 5)$ ($39{,}366 / 654 \approx 60.2$) is therefore a projection from smaller-scale measurements (Appendix~\ref{sec:reproducibility}) rather than a direct 20B observation.
