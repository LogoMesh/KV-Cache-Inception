...
% [REVISION | natbib-numbers-mode-D+ | 2026-05-12 | Explicit pre-load of natbib in numbers mode to resolve fatal "Bibliography not compatible with author-year citations" error from neurips_2026.sty's default natbib config. The bibliography (line 575+) uses bare `\bibitem{key}` entries (numeric format); neurips_2026.sty loads natbib in author-year mode which collides with the bibitem format and aborts PDF generation. Pre-loading natbib with the `numbers,sort&compress` options forces numeric citation mode before neurips_2026.sty's deferred natbib load runs. REMOVE THIS LINE at Day 8 ACL style switch (acl.sty + acl_natbib.bst provides its own numeric-mode natbib config). — pending audit]
...
\begin{itemize}[leftmargin=*]
    ...
    % [REVISION | §5.5-TDS-bullet-ref-cleanup-D+ | 2026-05-11 | Removed broken \ref{eq:mcts_reward} reference after §4.2.3 deletion (Decision 2). Inline body change only — the §5.5 Evaluation Metrics subsection as a whole is scheduled for rewrite per audit §5-6 on Day 7; this is a minimal stop-gap to keep compile clean. — pending audit]
    \item \textbf{Telemetry Divergence Score (TDS).} The Jensen--Shannon divergence between the H-Neuron stress and RepE projection signal distributions across layers; an internal cross-channel-disagreement diagnostic surfaced alongside the per-node telemetry traces. TDS is not tabulated as a primary evaluation outcome in our submission experiments and surfaces in the rollout traces and per-node telemetry but not as a results-table column.
