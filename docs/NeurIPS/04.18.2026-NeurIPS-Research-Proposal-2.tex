...
Recent work has moved beyond discrete token generation to perform search and reasoning directly in continuous representation spaces. COCONUT~\cite{hao2025coconut} bypasses discrete token decoding by feeding final-layer hidden states back as input embeddings, enabling emergent breadth-first latent tree search. CRSM~\cite{crsm2025} (an experimental prototype; not peer-reviewed) employs an asynchronous MCTS planner to directly edit the internal hidden states of a Mamba state-space model during generation. Both validate the viability of continuous latent search, but operate via recurrent hidden-state traversal rather than direct arithmetic mutation of a frozen transformer's KV-cache tensors.
...
\begin{hypothesis}[Orthogonal Escape]\label{hyp:orthogonal_escape}
...
% ╔══════════════════════════════════════════════════════════════════╗
% ║  DRAFT — HOLD — DO NOT REMOVE THIS BLOCK                        ║
% ║  Gap IDs: GAP-C1-05 (P1-B Finding 1) + P1-B Finding 4          ║
% ║                                                                  ║
% ║  Issue 1: This paragraph says "LAT procedure from Zou et al."   ║
% ║  but logomesh/whitebox.py uses difference-in-means, NOT PCA.    ║
% ║  Zou et al. (2023) use PCA as their primary method.             ║
% ║                                                                  ║
% ║  Issue 2: Code aggregates all L layers; Zou et al. recommend    ║
% ║  middle 20 layers only for the honesty probe signal.            ║
% ║                                                                  ║
% ║  Decision (2026-04-22, Josh): Hold paper fix until code fixed.  ║
% ║  Draft paper fix for Issue 1 (add after "LAT procedure"):       ║
% ║    "---using difference-in-means as a Phase 2 approximation;    ║
% ║     PCA-based LAT extraction is deferred to Phase 3---"         ║
% ║  Code fix owner: Max (whitebox.py) / Phase 3 (kv_mcts.py)      ║
% ╚══════════════════════════════════════════════════════════════════╝
...
% ║  Gap ID: GAP-C2-01                                              ║
% ║                                                                  ║
% ║  Issue: This section uses per-layer notation d_K^(ℓ), d_V^(ℓ)  ║
% ║  throughout (equations 8–11), implying steering vectors are      ║
% ║  applied independently at every layer ℓ ∈ {1,...,L}.            ║
% ║  Actual code (logomesh/kv_mcts.py): a SINGLE middle-layer       ║
% ║  steering vector is broadcast to all layers. There is no        ║
% ║  per-layer d_K^(ℓ) computation.                                 ║
% ║                                                                  ║
% ║  Decision (2026-04-23, Josh): Hold paper fix until Phase 3      ║
% ║  implements true per-layer steering.                             ║
% ║  Draft fix: qualify all per-layer notation as "Phase 2           ║
% ║  approximation: a single middle-layer vector broadcast to all    ║
% ║  L layers; per-layer independent steering deferred to Phase 3." ║
% ║  Code fix owner: Phase 3 (kv_mcts.py)                           ║
% ╚══════════════════════════════════════════════════════════════════╝
...
% ║  Gap ID: GAP-C2-03                                              ║
% ║                                                                  ║
% ║  Issue: This paragraph claims S' ≪ S mutated positions and      ║
% ║  ~50MB accumulator overhead. Actual code allocates FULL-SHAPE   ║
% ║  S×d accumulators (not sparse S'×d). For 20B, actual overhead  ║
% ║  ≈ M_KV (~40GB), not 50MB. Experiment 3 (memory profiling)     ║
% ║  will directly measure this and will falsify the 50MB claim     ║
% ║  if the code is not fixed first.                                ║
% ║                                                                  ║
% ║  Decision (2026-04-23, Josh): Hold paper fix until Phase 3      ║
% ║  implements sparse S'×d accumulators.                           ║
% ║  Draft fix: replace "approximately 50MB" with "approximately    ║
% ║  M_KV (Phase 2 uses full-shape accumulators; sparse S'×d        ║
% ║  allocation deferred to Phase 3)."                              ║
% ║  Code fix owner: Phase 3 (kv_mcts.py)                           ║
% ╚══════════════════════════════════════════════════════════════════╝
...
% ╔══════════════════════════════════════════════════════════════════╗
% ║  DRAFT — HOLD — DO NOT REMOVE THIS BLOCK                        ║
% ║  Gap ID: GAP-C1-05 (second location — see also §4.1 block)     ║
% ║                                                                  ║
% ║  Issue: This bullet says "RepE probe training via PCA."         ║
% ║  Actual code (logomesh/whitebox.py) uses difference-in-means,   ║
% ║  NOT PCA. The IMPL comment above marks Phase A as DONE, which   ║
% ║  makes this a live false claim — a reviewer reading IMPL tags   ║
% ║  will see "DONE" adjacent to a PCA claim the code doesn't       ║
% ║  fulfill.                                                        ║
% ║                                                                  ║
% ║  Decision (2026-04-22, Josh): Hold paper fix until code fixed.  ║
% ║  Draft fix: "via difference-in-means on contrastive pairs        ║
% ║  (Phase 2 approximation; PCA deferred to Phase 3)"             ║
% ║  Code fix owner: Max (whitebox.py)                              ║
% ╚══════════════════════════════════════════════════════════════════╝
...
Activation Steering & CAA (Contrastive Activation Addition) & Panickssery \& Rimsky et al., 2023~\cite{panickssery2023steering} \\
...
% ╔══════════════════════════════════════════════════════════════════╗
% ║  DRAFT — HOLD — DO NOT REMOVE THIS BLOCK                        ║
% ║  Gap ID: GAP-C2-03 + GAP-C2-04 (Proposition + comparison)      ║
% ║                                                                  ║
% ║  Issue 1 (GAP-C2-03): Proposition assumes sparse S'×d           ║
% ║  accumulators. Code uses full-shape S×d. K_acc in practice      ║
% ║  ≈ M_KV, not L·S'·d_model·4. The 40.05GB figure below is        ║
% ║  therefore wrong: actual is ~80GB (40GB base + ~40GB accum).    ║
% ║                                                                  ║
% ║  Issue 2 (GAP-C2-04): Code stores a baseline clone of M_KV to  ║
% ║  enable rollback. This adds another M_KV → minimum memory is   ║
% ║  2×M_KV before any accumulator overhead. The proposition        ║
% ║  ("independent of branching factor b") is correct in theory     ║
% ║  but the constant factor is wrong: ~2–3×M_KV, not ~1×M_KV.    ║
% ║                                                                  ║
% ║  This is the paper's flagship efficiency claim. Experiment 3    ║
% ║  will measure it directly. If experiments run before code is    ║
% ║  fixed, measured MER will contradict the proposition.           ║
% ║                                                                  ║
% ║  Decision (2026-04-23, Josh): Hold paper fix until Phase 3      ║
% ║  implements sparse accumulators and eliminates baseline clone.  ║
% ║  Draft fix: qualify proposition as Phase 2 approximation;       ║
% ║  update comparison numbers to reflect actual 2–3×M_KV floor.   ║
% ║  Code fix owner: Phase 3 (kv_mcts.py)                           ║
% ╚══════════════════════════════════════════════════════════════════╝
...
N.~Panickssery, N.~Rimsky, S.~Tong, et al.
