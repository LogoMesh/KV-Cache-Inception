...
\textbf{(i) Class-specific best-path selection (Table~\ref{tab:exp2-cartography-paths}).} The search selects heavy steering for MCQ classes at 1B ($\bar{\alpha} = 0.89$ for C2 ARC-Easy, $0.83$ for C3 TruthfulQA) but minimal-to-moderate steering for factual recall and continuation. At 3B the MCQ pattern inverts to minimal steering ($\bar{\alpha} = 0.22$ for C2, $0.28$ for C3), while factual recall shifts to moderate. The MCQ direction-flip across scales has magnitude $\Delta \approx 0.67$ (C2) and $\Delta \approx 0.55$ (C3); per-cell numbers including modal paths and modal shares are in Table~\ref{tab:exp2-cartography-paths}.
