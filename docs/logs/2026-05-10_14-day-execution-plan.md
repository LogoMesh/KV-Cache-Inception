...
| 9 | 05-18 | A | **Day-9 residue list pre-loaded items (filed 2026-05-12 from Day-4 P1 smoke-compile)**: (i) investigate scope of `natbib Warning: Author undefined for citation 'zou2023repe'` — soft warning ×2 in pass-2 log; non-fatal but worth diagnosing whether `\bibitem{zou2023repe}` lacks expected author-year metadata or whether the warning is intrinsic to numbers-mode natbib in our setup; (ii) confirm whether any `\citet{zou2023repe}` or `\citet{*}` usage exists in canonical TeX — if yes, the Author-undefined warning manifests as visible degraded inline-citation rendering; if no, the warning is purely log noise and can be suppressed. | residue list addendum |
